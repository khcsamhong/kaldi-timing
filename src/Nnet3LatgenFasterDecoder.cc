...
Nnet3LatgenFasterDecoder::Nnet3LatgenFasterDecoder() {
	...
	word_boundary_rxfilename_ = "word_boundary.int";
}
...
void Nnet3LatgenFasterDecoder::RegisterOptions(kaldi::OptionsItf &po) {
	...
	po.Register("nnet-in", &nnet3_rxfilename_,
	                "Path to nnet");
	po.Register("word-boundary-in", &word_boundary_rxfilename_,
					"Path to word boundary info");
	...
	word_boundary_opts_.Register(&po);
}
...
bool Nnet3LatgenFasterDecoder::Initialize(kaldi::OptionsItf &po) {
	...
	word_boundary_info_ = new kaldi::WordBoundaryInfo(word_boundary_opts_, word_boundary_rxfilename_);
...
bool Nnet3LatgenFasterDecoder::WordAlignLattice(const kaldi::CompactLattice &lat,
                      int32 max_states,
                      kaldi::CompactLattice *lat_out) {
	return kaldi::WordAlignLattice(lat, *trans_model_, *word_boundary_info_, max_states, lat_out);

//	kaldi::LatticeWordAligner aligner(lat, trans_model_, word_boundary_info_, max_states, lat_out);
